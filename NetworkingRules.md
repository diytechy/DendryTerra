...
* Now connect all the points:
    ...
    B. Now that initial downflows have been created, there should be multiple "chains" that contain various numbers of interconnected segments.  While any set of points or segments remain unconnected to the same or lower level segment (so stars must all eventually have a path to each-other, since there is no level below them):
        ii. Select the chain with the fewest number of segments (excluding the segment chain that includes segments created at lower levels), then select the lowest point on this chain, and attempt to create a connection using the "connection rules" below.  Again, the segment should be fully defined using the connection rules below before iterating onto the next point to prevent overlapping connections.
