...
public final class DendryBenchmark {
     ...
     */
    /**
     * Run one timed pass over the grid and return a BenchmarkResult.
     * If chunk caches are cold, this pass includes chunk build time.
     * If caches are warm, it measures hot lookup speed only.
     */
    public static BenchmarkResult timedPass(Sampler sampler, int gridSize, double worldScale) {
        long seed = 12345L;
        ...
        double avgNs = (double) totalTimeNs / totalSamples;
        return new BenchmarkResult(totalSamples, totalTimeMs, avgNs, avgNs / 1_000_000.0,
                totalSamples / (totalTimeMs / 1000.0), minValue, maxValue, sum / totalSamples);
    }

    /**
     * Run a cold pass followed by a warm pass and return both results.
     * Cold pass: chunk caches are empty, so time includes chunk build cost.
     * Warm pass: all chunks are cached, so time reflects pure lookup throughput.
     * Use this to separate "build amortization" cost from "query" cost.
     */
    public static BenchmarkResult[] benchmarkColdWarm(Sampler sampler, int gridSize, double worldScale) {
        LOGGER.info("Cold pass ({} samples)...", gridSize * gridSize);
        BenchmarkResult cold = timedPass(sampler, gridSize, worldScale);
        LOGGER.info("Warm pass ({} samples)...", gridSize * gridSize);
        BenchmarkResult warm = timedPass(sampler, gridSize, worldScale);
        return new BenchmarkResult[] { cold, warm };
    }

    /**
     * Original method: warmup N passes (untimed), then one timed pass.
     * The timed pass reflects warm-cache (lookup-only) throughput when warmupIterations >= 1.
     */
    public static BenchmarkResult benchmark(Sampler sampler, int gridSize, double worldScale, int warmupIterations) {
        ...
        LOGGER.info("Timing: {} samples...", gridSize * gridSize);
        return timedPass(sampler, gridSize, worldScale);
    }
